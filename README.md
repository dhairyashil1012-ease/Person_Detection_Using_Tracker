...
def cb_newpad(decodebin, decoder_src_pad, data):
    ...
    # if gstname.find("video") != -1:
    #     print("Video pad found:", gstname)

    # # STRICT CHECK FOR GPU MEMORY
    #     if not features.contains("memory:NVMM"):
    #         print("WARNING: Non-NVMM stream detected. Forcing compatibility...")
    ...
    if gstname.find("video") != -1:
        ...
        if not features.contains("memory:NVMM"):
            print("WARNING: Non-NVMM stream detected")

        if not bin_ghost_pad:
            sys.stderr.write("Failed to get ghost pad\n")
            return
...
def create_source_bin(index, uri):
    ...
    uri_decode_bin.set_property("caps", Gst.Caps.from_string(
...
def main(args):
    ...
    for i in range(number_sources):
        ...
        if uri_name.startswith("rtsp://"):
            is_live = True
        elif uri_name.startswith("file://") or uri_name.endswith(".mp4"):
            is_live = False
